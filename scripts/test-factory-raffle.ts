import 'dotenv/config'

import { parseEntries } from '../lib/cosmic-raffle/parseEntries'
import { MerkleTree } from '../lib/cosmic-raffle/merkle'
import { fetchCosmicSeed } from '../lib/cosmic-raffle/fetchCosmicSeed'
import { getAdminWallet, publicClient, FACTORY_ADDRESS, getWinningIndicesFromStandaloneContract } from '../lib/cosmic-raffle/contract'
import CosmicRaffleFactoryArtifact from '../contracts/CosmicRaffleFactory.json'
import CosmicRaffleInstanceArtifact from '../contracts/CosmicRaffleInstance.json'
import { encodeFunctionData, keccak256, encodePacked, decodeEventLog } from 'viem'

async function main() {
  console.log('\n🏁 Starting End-to-End Cosmic Raffle Factory & Standalone Instance Test...\n')
  console.log(`🏡 Factory Contract Address: ${FACTORY_ADDRESS}`)

  if (!FACTORY_ADDRESS) {
    throw new Error('NEXT_PUBLIC_COSMIC_RAFFLE_FACTORY_ADDRESS is missing in environment variables.')
  }

  // 1. Prepare raw input list
  const rawInput = `
    Alice
    Bob
    Charlie
    Dave
    Eve
  `
  const entries = parseEntries(rawInput)
  console.log(`👤 Parsed unique contestants:`, entries)

  // 2. Build Merkle Tree
  const tree = new MerkleTree(entries)
  const root = tree.getRoot()
  console.log(`🌳 Merkle Root computed: ${root}`)

  // 3. Deploy standalone raffle contract on-chain via Factory
  const wallet = getAdminWallet()
  const currentBlock = await publicClient.getBlockNumber()
  // Set commitBlock to currentBlock + 2 for fast testing
  const commitBlock = Number(currentBlock) + 2
  const winnerCount = 2

  console.log(`📡 Sending createRaffle transaction to Factory...`)
  console.log(`   - Title: E2E Factory Raffle Instance`)
  console.log(`   - Total Entries: ${entries.length}`)
  console.log(`   - Winner Count: ${winnerCount}`)
  console.log(`   - Commit Block: ${commitBlock} (Current Block: ${currentBlock})`)

  const callData = encodeFunctionData({
    abi: CosmicRaffleFactoryArtifact.abi,
    functionName: 'createRaffle',
    args: [
      wallet.account.address, // operator
      'E2E Factory Raffle Instance',
      root,
      BigInt(entries.length),
      BigInt(winnerCount),
      BigInt(commitBlock)
    ]
  })

  const txHash = await wallet.sendTransaction({
    account: wallet.account,
    to: FACTORY_ADDRESS,
    data: callData,
  })

  console.log(`⏳ Waiting for Factory transaction receipt (tx: ${txHash})...`)
  const receipt = await publicClient.waitForTransactionReceipt({ hash: txHash })
  console.log(`✅ Factory transaction confirmed in block ${receipt.blockNumber}!`)

  // 4. Extract newly deployed standalone contract address from logs
  let deployedRaffleAddress: `0x${string}` | null = null

  for (const log of receipt.logs) {
    try {
      const decoded = decodeEventLog({
        abi: CosmicRaffleFactoryArtifact.abi,
        data: log.data,
        topics: log.topics,
      })
      if (decoded.eventName === 'RaffleCreated' && decoded.args) {
        const args = decoded.args as { raffleAddress?: `0x${string}` }
        deployedRaffleAddress = args.raffleAddress as `0x${string}`
        break
      }
    } catch {
      // skip non-matching logs
    }
  }

  if (!deployedRaffleAddress) {
    throw new Error('❌ Failed to extract newly deployed CosmicRaffleInstance address from receipt logs.')
  }

  console.log(`🚀 Standalone Raffle Contract Deployed at: ${deployedRaffleAddress}`)

  // 5. Wait for commit block
  console.log(`⏳ Waiting for block ${commitBlock} to be mined...`)
  while (true) {
    const blockNum = await publicClient.getBlockNumber()
    if (blockNum >= BigInt(commitBlock)) {
      console.log(`✅ Target block ${commitBlock} reached (Current Block: ${blockNum})!`)
      break
    }
    console.log(`   - Still at block ${blockNum}, waiting 2 seconds...`)
    await new Promise((r) => setTimeout(r, 2000))
  }

  // 6. Fetch Cosmic Seed via SpaceComputer SDK cTRNG
  console.log(`🛰️ Fetching Cosmic Seed from SpaceComputer cTRNG setup...`)
  const seed = await fetchCosmicSeed(commitBlock)
  console.log(`🔮 Derived Verifiable Cosmic Seed: ${seed}`)

  // Pre-calculate winning indices off-chain using the seed
  const winningIndicesArray: number[] = []
  const totalEntries = entries.length
 
  // Replicate on-chain Fisher-Yates draw logic
  const tempSwaps = new Map<number, number>()
  for (let i = 0; i < winnerCount; i++) {
    const randHex = keccak256(encodePacked(['bytes32', 'uint256'], [seed, BigInt(i)]))
    const rand = BigInt(randHex)
    const j = i + Number(rand % BigInt(totalEntries - i))
 
    const valJ = tempSwaps.get(j) ?? j
    const valI = tempSwaps.get(i) ?? i
 
    tempSwaps.set(i, valJ)
    tempSwaps.set(j, valI)
 
    winningIndicesArray.push(valJ)
  }
 
  const winningUsernames = winningIndicesArray.map(idx => entries[idx])
  const proofs = winningIndicesArray.map(idx => tree.getProof(idx))

  // 7. Draw winners on the newly deployed standalone contract instance
  console.log(`📡 Sending drawWinners transaction to standalone contract address...`)
  const drawData = encodeFunctionData({
    abi: CosmicRaffleInstanceArtifact.abi,
    functionName: 'drawWinners',
    args: [
      seed,
      winningUsernames,
      proofs
    ]
  })

  const drawTx = await wallet.sendTransaction({
    account: wallet.account,
    to: deployedRaffleAddress,
    data: drawData,
  })

  console.log(`⏳ Waiting for drawWinners confirmation receipt (tx: ${drawTx})...`)
  const drawReceipt = await publicClient.waitForTransactionReceipt({ hash: drawTx })
  console.log(`✅ On-chain draw successfully confirmed in block ${drawReceipt.blockNumber}!`)

  // 8. Verify winners on-chain
  console.log(`📥 Fetching winning indices from Standalone Smart Contract...`)
  const winningIndices = await getWinningIndicesFromStandaloneContract(deployedRaffleAddress)
  console.log(`🏆 Winning indices drawn by contract:`, winningIndices)

  console.log(`\n🎉 --- VERIFIABLE STANDALONE DRAW RESULTS ---`)
  winningIndices.forEach((index, idx) => {
    const username = entries[index]
    const proof = tree.getProof(index)
    const valid = MerkleTree.verify(username, proof, root)
    console.log(`Winner #${idx + 1}: "${username}" (Ticket Index #${index})`)
    console.log(`  - Cryptographic Merkle Sibling Path: [${proof.map(p => `${p.slice(0, 10)}...`).join(', ')}]`)
    console.log(`  - Cryptographic Proof Valid: ${valid ? "✅ YES" : "❌ NO"}`)
  })
  console.log(`------------------------------------\n`)

  console.log(`🏁 End-to-End Factory Integration Test completed successfully!`)
}

main().catch((e) => {
  console.error('\n❌ E2E Integration Test failed:', e)
  process.exit(1)
})
