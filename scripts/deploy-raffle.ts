/**
 * deploy-raffle — deploy the CosmicRaffle contract.
 *
 * Usage: npx tsx scripts/deploy-raffle.ts
 *
 * Reads the compiled ABI + bytecode from contracts/CosmicRaffle.json
 * (generated by compile-raffle.ts). Uses self-contained viem setup.
 */
...
const artifactPath = path.resolve(
  __dirname,
  '../contracts/CosmicRaffle.json',
)

async function main() {
  console.log(`\n🏗  Deploying CosmicRaffle...`)
  ...
  if (!fs.existsSync(artifactPath)) {
    throw new Error(
      `Compiled artifact not found at ${artifactPath}.\n` +
        `Run: npx tsx scripts/compile-raffle.ts`,
    )
  }
  ...
  // Deploy (operator parameter is the admin address)
