/**
 * deploy-reputation — deploy the ReputationRegistry contract.
 *
 * Usage: pnpm reputation:deploy
 *
 * Reads the compiled ABI + bytecode from contracts/ReputationRegistry.json
 * (generated by compile-reputation.ts). Uses self-contained viem setup
 * (same pattern as mint-agents.ts).
 */
...
const artifactPath = path.resolve(
  __dirname,
  '../contracts/ReputationRegistry.json',
)

async function main() {
  console.log(`\n🏗  Deploying ReputationRegistry...`)
  console.log(`   operator = ${adminAccount.address}`)
  ...
  if (!fs.existsSync(artifactPath)) {
    throw new Error(
      `Compiled artifact not found at ${artifactPath}.\n` +
        `Run: npx tsx scripts/compile-reputation.ts`,
    )
  }
  ...
  console.log(`\n   Add to .env:`)
  console.log(`   REPUTATION_REGISTRY_ADDRESS=${contractAddress}\n`)
}
