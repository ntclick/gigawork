/**
 * verify-raffle — automatically verify the CosmicRaffle contract on ArcScan.
 *
 * Usage: npx tsx scripts/verify-raffle.ts
 */
...
async function main() {
  console.log(`\n🔍 Smart Contract Auto-Verification Initiated...`)
  ...
  // Retrieve operator constructor argument
  const adminAccount = privateKeyToAccount(ADMIN_PK as `0x${string}`)
  const operatorAddress = adminAccount.address
  
  // Format operator address constructor argument (padded to 32 bytes / 64 hex chars)
  const constructorArgs = operatorAddress.toLowerCase().replace('0x', '').padStart(64, '0')

  console.log(`   Contract Address:      ${CONTRACT_ADDRESS}`)
  console.log(`   Explorer Base URL:     ${EXPLORER}`)
  console.log(`   Constructor Operator:  ${operatorAddress}`)
  console.log(`   Constructor Hex Args:  ${constructorArgs}`)
  ...
  try {
    ...
    if (json.status !== '1') {
      // If already verified or other exception
