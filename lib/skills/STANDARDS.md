...
GigaWork is designed around. **Currently the workflow logic is shaped per
spec — actual on-chain calls are stubbed and ready to be enabled.**
...
### To upgrade to **full on-chain ERC-8183** (v3):

1. Replace `signDispatchTx` self-tx with a call to
   `GigaWorkCommerce.createJob(provider, evaluator, expiredAt, description, optParams)`
2. Each skill call becomes its own ERC-8183 job (escrow + provider attestation)
3. `nodes.status` transitions mirror on-chain `JobStatus` enum
4. Add USDC settle hooks: `complete(jobId, reason)` on success → 95% to skill
   owner wallet, 5% to platform treasury

The schema fields are already in place — just swap the function bodies.
